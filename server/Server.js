...
connection.connect(function(err) {
    ...
    console.log(connection.query("ALTER TABLE `reactMegaNews`.`news` ADD INDEX `fk_news_category_idx` (`news_category` ASC), ADD INDEX `fk_news_country_idx` (`news_country` ASC), ADD INDEX `fk_news_source_idx` (`news_source` ASC);"));
    console.log(connection.query("ALTER TABLE `reactMegaNews`.`news` ADD CONSTRAINT `fk_news_category` FOREIGN KEY (`news_category`) REFERENCES `reactMegaNews`.`category` (`category_id`) ON DELETE CASCADE ON UPDATE CASCADE, ADD CONSTRAINT `fk_news_country` FOREIGN KEY (`news_country`) REFERENCES `reactMegaNews`.`country` (`country_id`) ON DELETE CASCADE ON UPDATE CASCADE, ADD CONSTRAINT `fk_news_source` FOREIGN KEY (`news_source`) REFERENCES `reactMegaNews`.`source` (`source_id`) ON DELETE CASCADE ON UPDATE CASCADE;"));
  });
